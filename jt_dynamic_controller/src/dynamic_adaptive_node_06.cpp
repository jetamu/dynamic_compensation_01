...
public:
    ParameterEstimatorNode() : Node("parameter_estimator_node") {
        ...
        this->declare_parameter<double>("gamma_inv_1", 0.9);
        this->declare_parameter<double>("gamma_inv_2", 1.0);
        this->declare_parameter<double>("gamma_inv_3", 0.01);
        this->declare_parameter<double>("gamma_inv_4", 0.01);
        this->declare_parameter<double>("gamma_inv_5", 0.7);
        this->declare_parameter<double>("gamma_inv_6", 0.7);

        // Parámetros para Gamma_ (Fuga / Leakage)
        this->declare_parameter<double>("Gamma_1", 0.0007);
        this->declare_parameter<double>("Gamma_2", 0.003);
        this->declare_parameter<double>("Gamma_3", 0.000001);
        this->declare_parameter<double>("Gamma_4", 0.000001);
        this->declare_parameter<double>("Gamma_5", 0.0001);
        this->declare_parameter<double>("Gamma_6", 0.0001);
        ...
        // --- Configuración del filtro Alfa-Beta para Errores de Velocidad ---
        // Se recomienda alpha < 1.0 para que exista suavizado real frente al ruido.
        alpha_u_ = 0.5; // Ajustar entre 0.1 (muy suave, más retraso) y 0.9 (menos suave, menos retraso)
...
private:
    void control_loop() {
        ...
        // 5. Superficies de deslizamiento y Matriz G usando errores FILTRADOS
