...
public:
    ParameterEstimatorNode() : Node("parameter_estimator_node") {
        ...
        this->declare_parameter<double>("gamma_inv_1", 0.1);
        this->declare_parameter<double>("gamma_inv_2", 0.01);
        this->declare_parameter<double>("gamma_inv_3", 0.3);
        this->declare_parameter<double>("gamma_inv_4", 0.5);
        this->declare_parameter<double>("gamma_inv_5", 0.5);
        this->declare_parameter<double>("gamma_inv_6", 0.1);

        // Parámetros para Gamma_ (Fuga / Leakage)
        this->declare_parameter<double>("Gamma_1", 0.005);
        ...
        this->declare_parameter<double>("Gamma_5", 0.005);
        this->declare_parameter<double>("Gamma_6", 0.005);
...
private:
    void control_loop() {
        ...
        // Lógica de Reinicio por Inactividad (Sin usuario y detenido)
        if (std::abs(u_) < threshold_u_ && std::abs(w_) < threshold_w_ && std::abs(Fz_) < threshold_Fz_) {
            if (!was_reset_logged_) {
                theta_hat_ = theta_; // Forzar valores nominales
                RCLCPP_INFO(this->get_logger(), "Usuario desconectado y robot detenido. Parámetros reiniciados a base nominal.");
        ...
        // 1. Filtro Alfa-Beta para la fuerza (Idéntico al dinámico)
        ...
        // 2. Cálculo de aceleraciones derivadas de la admitancia
        uk_dot_ = Fx_filt_ / mu - du * u_ / mu;
        wk_dot_ = Tz_filt_ / mw - dw * w_ / mw;

        // 3. Errores de seguimiento cinemático (v_tilde en el paper)
        double u_tilde = uk_ - u_;
        double w_tilde = wk_ - w_;

        Eigen::Vector2d v_tilde(u_tilde, w_tilde);

        // 4. Superficies de deslizamiento (Sigma - Sincronizado exactamente con el dinámico)
        double sigma_1 = uk_dot_ + Ku1 * std::tanh(Ku2 * u_tilde);
        double sigma_2 = wk_dot_ + Kw1 * std::tanh(Kw2 * w_tilde);

        // 5. Matriz G (Regresor - Ecuación 8 del artículo)
        Eigen::Matrix<double, 2, 6> G;
        G << sigma_1, 0, -pow(w_, 2), u_, 0, 0,
             0, sigma_2, 0, 0, u_ * w_, w_;

        // Umbrales de Zona Muerta (Regla 3-Sigma calculada)
        double epsilon_u = 0.012;//0.03402/2; 
        double epsilon_w = 0.0294;//0.03584; 

        // 6. Ley de Actualización
        if (select_adaptation_ == 1) {
            
            // Se evalúa si el error supera el umbral estacionario
            if (std::abs(u_tilde) > epsilon_u || std::abs(w_tilde) > epsilon_w) {
                
                // Ecuación 17 del artículo (Ley adaptativa con Modificación Sigma estricta)
