...
public:
    ParameterEstimatorNode() : Node("parameter_estimator_node") {
        ...
        this->declare_parameter<double>("gamma_inv_1", 0.17);
        this->declare_parameter<double>("gamma_inv_2", 0.11);
        this->declare_parameter<double>("gamma_inv_3", 0.05);
        this->declare_parameter<double>("gamma_inv_4", 0.03);
        this->declare_parameter<double>("gamma_inv_5", 4.0);
        this->declare_parameter<double>("gamma_inv_6", 4.0);

        // Parámetros para Gamma_ (Fuga / Sigma modification del paper 2017)
        this->declare_parameter<double>("Gamma_1", 0.006);
        this->declare_parameter<double>("Gamma_2", 0.005);
        this->declare_parameter<double>("Gamma_3", 0.18);
        this->declare_parameter<double>("Gamma_4", 0.00006);
        this->declare_parameter<double>("Gamma_5", 0.01);
        this->declare_parameter<double>("Gamma_6", 0.007);
        ...
        // 1. Sincronizar topes de esfuerzo de tu Perfil Beta
        this->declare_parameter<double>("Ku1", 0.25);
        this->declare_parameter<double>("Kw1", 0.5);

        // Configuración del filtro Alfa-Beta para Fuerzas
        alpha_Fx_ = 1.0; 
        beta_Fx_ = pow(alpha_Fx_, 2) / (2 - alpha_Fx_);
        alpha_Tz_ = 1.0; 
        beta_Tz_ = pow(alpha_Tz_, 2) / (2 - alpha_Tz_);

        // Filtro Alfa-Beta para Errores de Velocidad 
        alpha_u_ = 1.0; 
        beta_u_ = pow(alpha_u_, 2) / (2 - alpha_u_);
        alpha_w_ = 1.0;
        beta_w_ = pow(alpha_w_, 2) / (2 - alpha_w_);

        // 2. Sincronizar sensibilidad (lambdas) de tu Perfil Beta
        ...
        // ... (código anterior de los lambdas) ...

        // 3. Declarar parámetros de la zona muerta (Dead-zone)
        this->declare_parameter<double>("epsilon_u", 0.012);
        this->declare_parameter<double>("epsilon_w", 0.0588);

        // Obtener los valores iniciales
        this->get_parameter("epsilon_u", epsilon_u);
        this->get_parameter("epsilon_w", epsilon_w);

        // Registrar el callback para detectar cambios...
    ...
    rcl_interfaces::msg::SetParametersResult parametersCallback(const std::vector<rclcpp::Parameter> &parameters)
    {
        ...
        for (const auto &parameter : parameters) {
            ...
            // --- AÑADE ESTO PARA LOS EPSILON ---
            else if (parameter.get_name() == "epsilon_u") {
                epsilon_u = parameter.as_double();
                RCLCPP_INFO(this->get_logger(), "Nuevo epsilon_u: %f", epsilon_u);
            }
            else if (parameter.get_name() == "epsilon_w") {
                epsilon_w = parameter.as_double();
                RCLCPP_INFO(this->get_logger(), "Nuevo epsilon_w: %f", epsilon_w);
            }
            // ... (código existente de gamma_inv_ y Gamma_) ...
            // gamma_inv_ y Gamma_ omitidos por brevedad en este bloque, se mantienen igual...
            else if (parameter.get_name() == "gamma_inv_1") { gamma_inv_.diagonal()[0] = parameter.as_double(); }
            else if (parameter.get_name() == "gamma_inv_2") { gamma_inv_.diagonal()[1] = parameter.as_double(); }
            else if (parameter.get_name() == "gamma_inv_3") { gamma_inv_.diagonal()[2] = parameter.as_double(); }
            else if (parameter.get_name() == "gamma_inv_4") { gamma_inv_.diagonal()[3] = parameter.as_double(); }
            else if (parameter.get_name() == "gamma_inv_5") { gamma_inv_.diagonal()[4] = parameter.as_double(); }
            else if (parameter.get_name() == "gamma_inv_6") { gamma_inv_.diagonal()[5] = parameter.as_double(); }
            else if (parameter.get_name() == "Gamma_1") { Gamma_.diagonal()[0] = parameter.as_double(); }
            else if (parameter.get_name() == "Gamma_2") { Gamma_.diagonal()[1] = parameter.as_double(); }
            else if (parameter.get_name() == "Gamma_3") { Gamma_.diagonal()[2] = parameter.as_double(); }
            else if (parameter.get_name() == "Gamma_4") { Gamma_.diagonal()[3] = parameter.as_double(); }
            else if (parameter.get_name() == "Gamma_5") { Gamma_.diagonal()[4] = parameter.as_double(); }
            else if (parameter.get_name() == "Gamma_6") { Gamma_.diagonal()[5] = parameter.as_double(); }
        }
        return result;
    }
...
private:
    void control_loop() {
        ...
        // 2. Filtro Alfa-Beta para la fuerza
        ...
        // 4. Errores de seguimiento cinemático crudos (Raw)
        double u_tilde_raw = uk_ - u_;
        double w_tilde_raw = wk_ - w_;

        // --- NUEVO: Filtro Alfa-Beta para los errores de velocidad ---
        
        // Para error de velocidad lineal (u_tilde)
        u_tilde_filt_ = u_tilde_filt_ + (u_tilde_p_filt_ * Ts_);
        double u_tilde_res_filtr = u_tilde_raw - u_tilde_filt_;
        u_tilde_filt_ = u_tilde_filt_ + alpha_u_ * u_tilde_res_filtr;
        u_tilde_p_filt_ = u_tilde_p_filt_ + (beta_u_ * u_tilde_res_filtr) / Ts_;

        // Para error de velocidad angular (w_tilde)
        w_tilde_filt_ = w_tilde_filt_ + (w_tilde_p_filt_ * Ts_);
        double w_tilde_res_filtr = w_tilde_raw - w_tilde_filt_;
        w_tilde_filt_ = w_tilde_filt_ + alpha_w_ * w_tilde_res_filtr;
        w_tilde_p_filt_ = w_tilde_p_filt_ + (beta_w_ * w_tilde_res_filtr) / Ts_;

        // Vector de velocidad usando las señales filtradas
        Eigen::Vector2d v_tilde(u_tilde_filt_, w_tilde_filt_);

        // 5. Superficies de deslizamiento y Matriz G de 2017
        double sigma_1 = uk_dot_ + Ku1 * std::tanh(Ku2 * u_tilde_filt_);
        double sigma_2 = wk_dot_ + Kw1 * std::tanh(Kw2 * w_tilde_filt_);

        // Matriz G basada en la Ecuación 14 del paper 2017:
        // Cruza la referencia (uk_, wk_) con las velocidades reales (u_, w_)
        Eigen::Matrix<double, 2, 6> G;
        G << sigma_1, 0.0,     -wk_ * w_,                 uk_, 0.0,      0.0,
             0.0,     sigma_2, (uk_ * w_ - u_ * wk_),     0.0, u_ * wk_, wk_;

       /* double epsilon_u = 0.012; 
        double epsilon_w = 0.0294; */

        // 6. Ley de Actualización 
        if (is_inactive || select_adaptation_ == 0) {
            theta_hat_ = theta_;
        } else {
            // Usamos las variables filtradas para la zona muerta adaptativa
            if (std::abs(u_tilde_filt_) > epsilon_u || std::abs(w_tilde_filt_) > epsilon_w) {
    ...
    // Variables para el filtro Alfa-Beta (Fuerzas)
    ...
    // --- NUEVO: Variables para el filtro Alfa-Beta (Errores de Velocidad) ---
    double u_tilde_filt_ = 0.0, u_tilde_p_filt_ = 0.0;
    double w_tilde_filt_ = 0.0, w_tilde_p_filt_ = 0.0;
    double alpha_u_, beta_u_, alpha_w_, beta_w_;
    ...
    // Umbrales ajustables
    const double threshold_u_ = 0.02; 
    const double threshold_w_ = 0.02; 
    const double threshold_Fz_ = 5.0; 

    // Variables de referencia cinemática
